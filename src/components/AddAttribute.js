...
export default class AddTable extends Component {
    ...
    getTable() {
        axios.get("http://db-management-system.herokuapp.com/api/tables/" + this.state.tableId, {
            headers: {
    ...
    createUserRequest(firstName, type, maxLength) {
        ...
        let _this = this;
        this.setState({isLoading: true});
        axios.post(`http://db-management-system.herokuapp.com/api/attribute?name=${firstName}&type=${type}&maxLength=${maxLength}&headerId=${_this.state.table.header.id}`, {
            headers: {
                'Content-Type': 'application/json'
            }
        })
